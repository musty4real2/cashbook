...
$fetch="SELECT * FROM `cr` WHERE MONTH(`entry_date`)= MONTH(NOW()) AND YEAR(`entry_date`)=YEAR(NOW()) AND  `senders_bank`='{$_GET['bank']}' AND `senders_account_number`='{$_GET['account']}'  ORDER BY `entry_date` ASC LIMIT $start, $display ";
...
<p class="empty">Below are Monthly Record  of Schedule prepared</p>

  <center><table width='100%' border='0'style='margin:auto;' class="table">
...
if ($current_page != 1) {
 echo '<center><a href="monthly_log.php?s=' .
...
for ($i = 1; $i <= $pages; $i++) {
 if ($i != $current_page) {
 echo '<a href="monthly_log.php?s=' .
(($display * ($i - 1))) . '&p=' .
$pages . '&bank='.$bank.'&bank='.$_GET['bank'].'&account='.$_GET['account'].'&accountn='.$_GET['accountn'].'">' . $i . '</a> ';
 } else {
 echo $i . ' ';
}
 }
...
if ($current_page != $pages) {
 echo '<center><a href="monthly_log.php?s=' .
($start + $display) . '&p=' . $pages .'&bank='.$_GET['bank'].'&account='.$_GET['account'].'&accountn='.$_GET['accountn'].
'&bank='.$bank.'&bank='.$_GET['bank'].'&account='.$_GET['account'].'&accountn='.$_GET['accountn'].'">Next</a></center>';
 }
